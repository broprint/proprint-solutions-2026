...
export function ProductGallery({ product }: { product: Product }) {
  ...
  return <div>
    <button type="button" onClick={() => setLightboxOpen(true)} aria-label={tr(`Enlarge ${product.name} image`,`تكبير صورة ${product.name}`)} className="group relative block h-[360px] w-full cursor-zoom-in overflow-hidden rounded-[2rem] border border-slate-200 bg-white text-left shadow-sm md:h-[460px]">
      ...
      <span className="absolute bottom-4 right-4 inline-flex items-center gap-2 rounded-full bg-slate-900/80 px-4 py-2 text-xs font-black text-white shadow-lg backdrop-blur transition group-hover:bg-slate-900"><Maximize2 size={15}/> {tr('Click to enlarge','اضغط للتكبير')}</span>
    </button>

    {gallery.length>1&&<div className="mt-4 grid grid-cols-5 gap-3">{gallery.map((image,index)=>{const selected=image===selectedImage;return <button key={`${image}-${index}`} type="button" onClick={()=>{setSelectedImage(image);setZoom(1);}} aria-label={tr(`View product image ${index+1}`,`عرض صورة المنتج ${index+1}`)} className={`relative aspect-square overflow-hidden rounded-xl border bg-white transition ${selected?'border-[#0b5cff] ring-2 ring-blue-100':'border-slate-200 hover:border-slate-400'}`}><Image src={image} alt={`${product.imageAlt || product.name} ${index+1}`} fill sizes="120px" className="object-contain p-2"/></button>;})}</div>}

    {lightboxOpen&&selectedImage&&<div role="dialog" aria-modal="true" aria-label={tr(`${product.name} image viewer`,`عارض صور ${product.name}`)} className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/95 p-4 md:p-8" onClick={closeLightbox}>
      <div className="absolute right-4 top-4 z-20 flex items-center gap-2" onClick={event=>event.stopPropagation()}>
        <button type="button" onClick={()=>setZoom(value=>Math.max(1,value-0.5))} disabled={zoom<=1} aria-label={tr('Zoom out','تصغير')} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-slate-900 shadow disabled:opacity-40"><Minus size={20}/></button>
        ...
        <button type="button" onClick={()=>setZoom(value=>Math.min(3,value+0.5))} disabled={zoom>=3} aria-label={tr('Zoom in','تكبير')} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-slate-900 shadow disabled:opacity-40"><Plus size={20}/></button>
        <button type="button" onClick={closeLightbox} aria-label={tr('Close image viewer','إغلاق عارض الصور')} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-slate-900 shadow"><X size={22}/></button>
      </div>

      {gallery.length>1&&<><button type="button" onClick={event=>{event.stopPropagation();showImage(selectedIndex-1);}} aria-label={tr('Previous image','الصورة السابقة')} className="absolute left-3 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-900 shadow md:left-6"><ChevronLeft size={26}/></button><button type="button" onClick={event=>{event.stopPropagation();showImage(selectedIndex+1);}} aria-label={tr('Next image','الصورة التالية')} className="absolute right-3 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-900 shadow md:right-6"><ChevronRight size={26}/></button></>}
