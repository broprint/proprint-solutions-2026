...
export function ProductVisual({ icon, compact=false, image, imageAlt='' }: { icon:keyof typeof map; compact?:boolean; image?:string; imageAlt?:string }) {
  ...
  return <div className={`product-visual relative overflow-hidden ${compact ? 'h-44' : 'h-60'}`}>
    ...
    <div className="absolute left-4 top-4 z-20 rounded-full border border-blue-100 bg-white/90 px-3 py-1 text-[9px] font-black uppercase tracking-[.12em] text-[#0b5cff] shadow-sm backdrop-blur">{labels[icon]}</div>
    <div className="relative z-10 flex h-full items-center justify-center">
      <div className={`relative flex items-center justify-center rounded-[2rem] border border-white/70 bg-white/80 shadow-2xl shadow-blue-900/10 backdrop-blur ${compact ? 'h-28 w-40' : 'h-36 w-48'}`}>
        <div className="absolute -inset-1 rounded-[2rem] bg-gradient-to-br from-blue-400/20 via-transparent to-orange-400/20" />
        <Icon className="relative z-10 text-[#0b5cff] drop-shadow-xl" size={compact ? 68 : 88} strokeWidth={1.35}/>
      </div>
    </div>
