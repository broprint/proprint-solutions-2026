...
export function ProductCard({ product }: { product:Product }) {
  ...
  return <article className="group overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <div className="p-5">
      ...
      <div className="mt-5 flex items-end justify-between gap-3"><div><div className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{quoteOnly?'Business pricing':'Prototype price'}</div><b className="mt-1 block text-lg">{product.price}</b></div><Link href={`/products/${product.slug}`} className="rounded-full bg-[#071525] px-4 py-2.5 text-xs font-black text-white transition group-hover:bg-[#0b5cff]">Details <ArrowRight className="ml-1 inline" size={13}/></Link></div>
    </div>
  </article>;
}
