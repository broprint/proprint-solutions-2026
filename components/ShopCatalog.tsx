...
export function ShopCatalog({ products, initialCategory = 'All' }: { products: Product[]; initialCategory?: string }) {
  ...
  return <>
    ...
    <section className="py-14"><div className="container"><div className="grid gap-7 lg:grid-cols-[260px_1fr]">
      ...
      <div><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-3xl font-black tracking-[-.03em]">Shop products</h2><p className="mt-1 text-sm text-slate-500">{filtered.length} product{filtered.length === 1 ? '' : 's'} shown · ProPrint-managed catalog</p></div><button onClick={() => setFiltersOpen((value) => !value)} className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 px-4 py-2 text-sm font-bold lg:hidden"><SlidersHorizontal size={16}/> Filters</button></div>
