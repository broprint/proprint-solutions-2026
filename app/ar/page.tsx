...
export default function ArabicHome(){
 return <main dir="rtl" lang="ar" className="text-right">
   ...
   <div className="container relative grid min-h-[620px] items-center gap-12 py-16 lg:grid-cols-2">
    <div>
     ...
     <div className="mt-8 flex flex-wrap gap-3">
      <Link href="/shop" className="rounded-full bg-[#0b5cff] px-6 py-3.5 font-black text-white">تصفح المنتجات <ArrowLeft className="mr-2 inline" size={17}/></Link>
      <Link href="/quote" className="rounded-full bg-[#f47b20] px-6 py-3.5 font-black text-white">اطلب عرض سعر</Link>
      <Link href="/service" className="rounded-full border border-white/20 bg-white/10 px-6 py-3.5 font-black text-white">طلب صيانة</Link>
     </div>
  ...
  <section className="py-20"><div className="container"><div className="text-[11px] font-black tracking-wide text-[#0b5cff]">تسوّق حسب الفئة</div><h2 className="mt-3 text-4xl font-black">كل احتياجاتك التقنية</h2><p className="mt-4 max-w-2xl leading-8 text-slate-600">مجموعة واسعة من منتجات تقنية المعلومات مدعومة بخدمات ProPrint في المبيعات والتركيب والدعم الفني.</p><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map(([Icon,title,text])=>{const I=Icon;return <Link href="/shop" key={title} className="category-card group"><div className="category-icon"><I size={29}/></div><div><h3 className="text-lg font-black">{title}</h3><p className="mt-1 text-sm leading-7 text-slate-500">{text}</p><span className="mt-4 inline-block text-xs font-black text-[#0b5cff]">عرض المنتجات ←</span></div></Link>})}</div></div></section>
  <section className="bg-[#061321] py-20 text-white"><div className="container grid gap-10 lg:grid-cols-2"><div><div className="text-[11px] font-black text-blue-300">مركز خدمات ProPrint</div><h2 className="mt-3 text-4xl font-black">نحن ندعم ما نبيعه — وأكثر.</h2><p className="mt-5 leading-8 text-slate-300">خدمات فنية احترافية للطابعات والبلوتر وأجهزة الكمبيوتر ومعدات تقنية المعلومات للشركات.</p><Link href="/service" className="mt-7 inline-flex rounded-full bg-[#0b5cff] px-6 py-3 font-black">اطلب خدمة</Link></div><div className="grid gap-3 sm:grid-cols-2">{['صيانة الطابعات والبلوتر','دعم أجهزة اللابتوب والكمبيوتر','التركيب والإعداد','دعم الضمان وقطع الغيار','الصيانة الوقائية','عقود الصيانة السنوية AMC'].map(x=><div key={x} className="rounded-2xl border border-white/10 bg-white/[.05] p-4 font-bold"><CheckCircle2 className="ml-2 inline text-[#57a7ff]" size={17}/>{x}</div>)}</div></div></section>
  <section className="py-20"><div className="container"><div className="text-[11px] font-black text-[#0b5cff]">تقنية المعلومات وخدمات الشركات</div><h2 className="mt-3 text-4xl font-black">قدرات ميدانية لبيئات الشركات.</h2><p className="mt-5 max-w-3xl leading-8 text-slate-600">دعم تقنية المعلومات في الموقع، والهندسة الميدانية، وخدمات الأجهزة، والنشر والتجهيز لبيئات الشركات في جميع أنحاء الكويت.</p><Link href="/enterprise" className="mt-7 inline-flex rounded-full bg-[#061321] px-6 py-3.5 font-black text-white">خدمات الشركات <ArrowLeft className="mr-2"/></Link></div></section>
 </main>
}
