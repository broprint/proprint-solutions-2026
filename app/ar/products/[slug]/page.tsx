...
export default async function ArabicProductPage({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params; const {getPublishedProducts,toStoreProduct}=await import('@/lib/products'); const rows=await getPublishedProducts(); const row=rows.find(p=>p.slug===slug); if(!row)notFound(); const product=toStoreProduct(row); const displayName=row.name_ar?.trim()||product.name; const displaySpecs=Array.isArray(row.specifications_ar)&&row.specifications_ar.length?row.specifications_ar:product.specs; const displayDescription=row.description_ar?.trim()||row.short_description_ar?.trim()||'منتج تقني للأعمال مع إمكانية الاستفادة من خدمات ProPrint للتركيب والإعداد ودعم ما بعد البيع في الكويت.';
 ...
 return <main dir="rtl" lang="ar" className="text-right">
  <section className="border-b bg-white py-5"><div className="container flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500"><Link href="/ar/shop" className="hover:text-[#0b5cff]">المتجر</Link><span>←</span><span>{product.category}</span><span>←</span><span className="text-slate-900">{displayName}</span></div></section>
  <section className="py-12"><div className="container grid gap-10 lg:grid-cols-[1.05fr_.95fr]"><ProductGallery product={product}/><div>
   ...
   <h1 className="mt-3 text-4xl font-black md:text-5xl">{displayName}</h1>
   <p className="mt-4 max-w-xl text-sm leading-8 text-slate-600">{displayDescription}</p>
   ...
   <div className="mt-7 grid gap-3 sm:grid-cols-2">{displaySpecs.map(x=><div key={x} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold"><CheckCircle2 size={17} className="text-[#0b5cff]"/>{x}</div>)}</div>
